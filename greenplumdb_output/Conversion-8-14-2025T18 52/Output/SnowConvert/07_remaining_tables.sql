...
CREATE TABLE trading.ib_deals (
    deal_id INTEGER PRIMARY KEY IDENTITY,
    ...
    deal_team ARRAY /*** SSC-FDM-PG0016 - STRONGLY TYPED ARRAY 'TEXT[]' TRANSFORMED TO ARRAY WITHOUT TYPE CHECKING ***/,
...
CREATE TABLE loans.syndicated_loans (
    syndicated_loan_id INTEGER PRIMARY KEY IDENTITY,
    loan_id INTEGER REFERENCES loans.loans (loan_id),
    facility_name VARCHAR(200) NOT NULL,
    total_facility_amount DECIMAL(20,2) NOT NULL,
    arranger_bank VARCHAR(200),
    participant_banks ARRAY /*** SSC-FDM-PG0016 - STRONGLY TYPED ARRAY 'TEXT[]' TRANSFORMED TO ARRAY WITHOUT TYPE CHECKING ***/,
...
CREATE TABLE trading.fx_forwards (
    fx_forward_id INTEGER PRIMARY KEY IDENTITY,
    derivative_id INTEGER REFERENCES trading.derivatives (derivative_id),
...
--** SSC-FDM-GP0001 - THE PERFORMANCE OF THE CLUSTER BY MAY VARY COMPARED TO THE PERFORMANCE OF DISTRIBUTED BY **
CLUSTER BY (fx_forward_id)
COMMENT = '{ "origin": "sf_sc", "name": "snowconvert", "version": {  "major": 1,  "minor": 16,  "patch": "1.0" }, "attributes": {  "component": "greenplum",  "convertedOn": "08/14/2025",  "domain": "snowflake" }}'
;

-- 136. Commodity Trading
CREATE TABLE trading.commodity_trades (
    commodity_trade_id INTEGER PRIMARY KEY IDENTITY,
    ...
    settlement_type VARCHAR(20), -- CASH, PHYSICAL
    quality_specifications VARIANT,
...
--** SSC-FDM-GP0001 - THE PERFORMANCE OF THE CLUSTER BY MAY VARY COMPARED TO THE PERFORMANCE OF DISTRIBUTED BY **
CLUSTER BY (commodity_trade_id)
COMMENT = '{ "origin": "sf_sc", "name": "snowconvert", "version": {  "major": 1,  "minor": 16,  "patch": "1.0" }, "attributes": {  "component": "greenplum",  "convertedOn": "08/14/2025",  "domain": "snowflake" }}'
;

-- 137. Regulatory Capital
CREATE TABLE risk.regulatory_capital (
    capital_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE risk.liquidity_ratios (
    liquidity_ratio_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE risk.ecl_calculations (
    ...
    macroeconomic_scenarios VARIANT,
...
CREATE TABLE compliance.customer_due_diligence (
    cdd_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE loans.trade_finance (
    trade_finance_id INTEGER PRIMARY KEY IDENTITY,
    ...
    documents_required ARRAY /*** SSC-FDM-PG0016 - STRONGLY TYPED ARRAY 'TEXT[]' TRANSFORMED TO ARRAY WITHOUT TYPE CHECKING ***/,
...
CREATE TABLE trading.fx_positions (
    fx_position_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE trading.mutual_funds (
    fund_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE trading.mutual_fund_transactions (
    mf_transaction_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE core.insurance_products (
    insurance_product_id INTEGER PRIMARY KEY IDENTITY,
...
--** SSC-FDM-GP0001 - THE PERFORMANCE OF THE CLUSTER BY MAY VARY COMPARED TO THE PERFORMANCE OF DISTRIBUTED BY **
CLUSTER BY (insurance_product_id)
COMMENT = '{ "origin": "sf_sc", "name": "snowconvert", "version": {  "major": 1,  "minor": 16,  "patch": "1.0" }, "attributes": {  "component": "greenplum",  "convertedOn": "08/14/2025",  "domain": "snowflake" }}'
;

-- 148. Insurance Policies
CREATE TABLE core.insurance_policies (
    policy_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE core.pension_plans (
    pension_plan_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE trading.wealth_management_accounts (
    wealth_account_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE audit.database_performance (
    metric_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE audit.application_logs (
    log_id INTEGER PRIMARY KEY IDENTITY,
...
--** SSC-FDM-GP0001 - THE PERFORMANCE OF THE CLUSTER BY MAY VARY COMPARED TO THE PERFORMANCE OF DISTRIBUTED BY **
CLUSTER BY (application_name)
COMMENT = '{ "origin": "sf_sc", "name": "snowconvert", "version": {  "major": 1,  "minor": 16,  "patch": "1.0" }, "attributes": {  "component": "greenplum",  "convertedOn": "08/14/2025",  "domain": "snowflake" }}'
;

-- 153. Data Quality Metrics
CREATE TABLE analytics.data_quality_metrics (
    dq_metric_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE core.configuration_items (
    config_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE loans.credit_scoring_variables (
    variable_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE risk.market_risk_factors (
    factor_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE analytics.economic_indicators (
    indicator_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE trading.hedge_fund_strategies (
    strategy_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE trading.alternative_investments (
    alt_investment_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE trading.bond_analytics (
    bond_analytics_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE analytics.esg_scores (
    esg_score_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE trading.algo_trading_strategies (
    ...
    target_securities ARRAY /*** SSC-FDM-PG0016 - STRONGLY TYPED ARRAY 'TEXT[]' TRANSFORMED TO ARRAY WITHOUT TYPE CHECKING ***/,
    risk_limits VARIANT,
    performance_metrics VARIANT,
    backtest_results VARIANT,
...
CREATE TABLE trading.robo_advisory (
    robo_account_id INTEGER PRIMARY KEY IDENTITY,
    customer_id INTEGER REFERENCES core.customers (customer_id),
    risk_questionnaire_id INTEGER,
    risk_score INTEGER,
    investment_goal VARCHAR(100),
    time_horizon INTEGER, -- years
    target_allocation VARIANT,
    current_allocation VARIANT,
...
CREATE TABLE trading.crypto_holdings (
    crypto_holding_id INTEGER PRIMARY KEY IDENTITY,
    portfolio_id INTEGER REFERENCES trading.portfolios (portfolio_id),
...
--** SSC-FDM-0007 - MISSING DEPENDENT OBJECT "pg_tables" **
CREATE OR REPLACE VIEW analytics.table_count_verification
COMMENT = '{ "origin": "sf_sc", "name": "snowconvert", "version": {  "major": 1,  "minor": 16,  "patch": "1.0" }, "attributes": {  "component": "greenplum",  "convertedOn": "08/14/2025",  "domain": "snowflake" }}'
AS
SELECT
    schemaname,

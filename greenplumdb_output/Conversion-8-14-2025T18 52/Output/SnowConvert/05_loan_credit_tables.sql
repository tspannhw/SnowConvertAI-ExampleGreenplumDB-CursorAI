...
USE SCHEMA loans /*, core, risk, public*/;

-- 76. Loan Products
CREATE TABLE loans.loan_products (
    loan_product_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE loans.loan_applications (
    application_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE loans.credit_scoring_models (
    model_id INTEGER PRIMARY KEY IDENTITY,
    model_name VARCHAR(100) NOT NULL,
    model_version VARCHAR(20) NOT NULL,
    model_type VARCHAR(50) NOT NULL, -- FICO, CUSTOM, BEHAVIORAL
    description TEXT,
    algorithm VARCHAR(50),
    variables_used ARRAY /*** SSC-FDM-PG0016 - STRONGLY TYPED ARRAY 'TEXT[]' TRANSFORMED TO ARRAY WITHOUT TYPE CHECKING ***/,
...
CREATE TABLE loans.credit_scores (
    credit_score_id INTEGER PRIMARY KEY IDENTITY,
    customer_id INTEGER REFERENCES core.customers (customer_id),
    application_id INTEGER REFERENCES loans.loan_applications (application_id),
    model_id INTEGER REFERENCES loans.credit_scoring_models (model_id),
    score_value INTEGER NOT NULL,
    score_date DATE NOT NULL,
    score_reason_codes ARRAY /*** SSC-FDM-PG0016 - STRONGLY TYPED ARRAY 'TEXT[]' TRANSFORMED TO ARRAY WITHOUT TYPE CHECKING ***/,
    bureau_name VARCHAR(50),
    bureau_reference VARCHAR(100),
    factors_affecting_score VARIANT,
...
CREATE TABLE loans.underwriting_criteria (
    criteria_id INTEGER PRIMARY KEY IDENTITY,
    loan_product_id INTEGER REFERENCES loans.loan_products (loan_product_id),
...
CREATE TABLE loans.underwriting_decisions (
    decision_id INTEGER PRIMARY KEY IDENTITY,
    application_id INTEGER REFERENCES loans.loan_applications (application_id),
    ...
    conditions ARRAY /*** SSC-FDM-PG0016 - STRONGLY TYPED ARRAY 'TEXT[]' TRANSFORMED TO ARRAY WITHOUT TYPE CHECKING ***/,
    decline_reasons ARRAY /*** SSC-FDM-PG0016 - STRONGLY TYPED ARRAY 'TEXT[]' TRANSFORMED TO ARRAY WITHOUT TYPE CHECKING ***/,
...
CREATE TABLE loans.loan_schedules (
    schedule_id INTEGER PRIMARY KEY IDENTITY,
    loan_id INTEGER REFERENCES loans.loans (loan_id),
...
CREATE TABLE loans.collateral_types (
    collateral_type_id INTEGER PRIMARY KEY IDENTITY,
...
--** SSC-FDM-GP0001 - THE PERFORMANCE OF THE CLUSTER BY MAY VARY COMPARED TO THE PERFORMANCE OF DISTRIBUTED BY **
CLUSTER BY (collateral_type_id)
COMMENT = '{ "origin": "sf_sc", "name": "snowconvert", "version": {  "major": 1,  "minor": 16,  "patch": "1.0" }, "attributes": {  "component": "greenplum",  "convertedOn": "08/14/2025",  "domain": "snowflake" }}'
;

-- 87. Loan Collateral
CREATE TABLE loans.loan_collateral (
    collateral_id INTEGER PRIMARY KEY IDENTITY,
    loan_id INTEGER REFERENCES loans.loans (loan_id),
...
CREATE TABLE loans.loan_guarantors (
    guarantor_id INTEGER PRIMARY KEY IDENTITY,
    loan_id INTEGER REFERENCES loans.loans (loan_id),
...
CREATE TABLE loans.loan_modifications (
    modification_id INTEGER PRIMARY KEY IDENTITY,
    loan_id INTEGER REFERENCES loans.loans (loan_id),
...
CREATE TABLE loans.delinquency_tracking (
    delinquency_id INTEGER PRIMARY KEY IDENTITY,
    loan_id INTEGER REFERENCES loans.loans (loan_id),
...
CREATE TABLE loans.charge_offs (
    charge_off_id INTEGER PRIMARY KEY IDENTITY,
    loan_id INTEGER REFERENCES loans.loans (loan_id),
...
CREATE TABLE loans.recovery_tracking (
    recovery_id INTEGER PRIMARY KEY IDENTITY,
...
--** SSC-FDM-GP0001 - THE PERFORMANCE OF THE CLUSTER BY MAY VARY COMPARED TO THE PERFORMANCE OF DISTRIBUTED BY **
CLUSTER BY (charge_off_id)
COMMENT = '{ "origin": "sf_sc", "name": "snowconvert", "version": {  "major": 1,  "minor": 16,  "patch": "1.0" }, "attributes": {  "component": "greenplum",  "convertedOn": "08/14/2025",  "domain": "snowflake" }}'
;

-- 93. Loan Provisions
CREATE TABLE loans.loan_provisions (
    provision_id INTEGER PRIMARY KEY IDENTITY,
    loan_id INTEGER REFERENCES loans.loans (loan_id),
...
CREATE TABLE loans.loan_pricing (
    pricing_id INTEGER PRIMARY KEY IDENTITY,
    loan_product_id INTEGER REFERENCES loans.loan_products (loan_product_id),
...
CREATE TABLE loans.servicing_rights (
    servicing_rights_id INTEGER PRIMARY KEY IDENTITY,
    loan_id INTEGER REFERENCES loans.loans (loan_id),
...
CREATE TABLE loans.escrow_accounts (
    escrow_account_id INTEGER PRIMARY KEY IDENTITY,
    loan_id INTEGER REFERENCES loans.loans (loan_id),
    account_id INTEGER REFERENCES core.accounts (account_id),
...
CREATE TABLE loans.escrow_transactions (
    escrow_transaction_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE loans.loan_performance_analytics (
    ...
    early_warning_signals ARRAY /*** SSC-FDM-PG0016 - STRONGLY TYPED ARRAY 'TEXT[]' TRANSFORMED TO ARRAY WITHOUT TYPE CHECKING ***/,
    recommended_actions ARRAY /*** SSC-FDM-PG0016 - STRONGLY TYPED ARRAY 'TEXT[]' TRANSFORMED TO ARRAY WITHOUT TYPE CHECKING ***/,
    calculated_by INTEGER REFERENCES core.employees (employee_id),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP()
)
--** SSC-FDM-GP0001 - THE PERFORMANCE OF THE CLUSTER BY MAY VARY COMPARED TO THE PERFORMANCE OF DISTRIBUTED BY **
...
CREATE TABLE loans.credit_lines (
    credit_line_id INTEGER PRIMARY KEY IDENTITY,

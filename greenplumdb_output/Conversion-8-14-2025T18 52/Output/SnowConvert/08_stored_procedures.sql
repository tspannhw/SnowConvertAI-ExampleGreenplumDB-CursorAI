...
--** SSC-FDM-0007 - MISSING DEPENDENT OBJECTS "gen_salt", "crypt" **
    CREATE OR REPLACE PROCEDURE core.create_customer (
...
SELECT 'CUST' || LPAD(core.customers_customer_id_seq.nextval /*** SSC-FDM-PG0009 - THE SEQUENCE NEXTVAL PROPERTY SNOWFLAKE DOES NOT GUARANTEE GENERATING SEQUENCE NUMBERS WITHOUT GAPS. ***/ ::TEXT, 8, '0') INTO
        : v_customer_number;
...
) RETURNING customer_id INTO v_customer_id !!!RESOLVE EWI!!! /*** SSC-EWI-0073 - PENDING FUNCTIONAL EQUIVALENCE REVIEW FOR 'InsertIntoBody' NODE ***/!!!;

-- Insert address
INSERT INTO core.customer_addresses (
    customer_id, address_line1, city_id, is_primary
) VALUES (:v_customer_id, : p_address_line1, : p_city_id, TRUE
);
...
!!!RESOLVE EWI!!! /*** SSC-EWI-0068 - USER DEFINED FUNCTION WAS TRANSFORMED TO SNOWFLAKE PROCEDURE ***/!!!
CREATE OR REPLACE PROCEDURE core.create_account (
p_customer_id INTEGER,
p_account_type_id INTEGER,
p_branch_id INTEGER,
p_currency_code CHAR(3),
p_initial_deposit DECIMAL(18,2) DEFAULT 0 !!!RESOLVE EWI!!! /*** SSC-EWI-0073 - PENDING FUNCTIONAL EQUIVALENCE REVIEW FOR 'ParameterDefaultExpr' NODE ***/!!!)
RETURNS INTEGER
LANGUAGE SQL
COMMENT = '{ "origin": "sf_sc", "name": "snowconvert", "version": {  "major": 1,  "minor": 16,  "patch": "1.0" }, "attributes": {  "component": "greenplum",  "convertedOn": "08/14/2025",  "domain": "snowflake" }}'
AS $$
...
SELECT
        : p_branch_id::TEXT || LPAD(core.accounts_account_id_seq.nextval /*** SSC-FDM-PG0009 - THE SEQUENCE NEXTVAL PROPERTY SNOWFLAKE DOES NOT GUARANTEE GENERATING SEQUENCE NUMBERS WITHOUT GAPS. ***/ ::TEXT, 10, '0') INTO
        : v_account_number;
...
) RETURNING account_id INTO v_account_id !!!RESOLVE EWI!!! /*** SSC-EWI-0073 - PENDING FUNCTIONAL EQUIVALENCE REVIEW FOR 'InsertIntoBody' NODE ***/!!!;
...
IF (:p_initial_deposit > 0) THEN
    ...
    ) RETURNING gl_transaction_id INTO v_gl_transaction_id !!!RESOLVE EWI!!! /*** SSC-EWI-0073 - PENDING FUNCTIONAL EQUIVALENCE REVIEW FOR 'InsertIntoBody' NODE ***/!!!;

    -- Debit Cash GL Account
    INSERT INTO core.gl_transaction_details (
        gl_transaction_id, gl_account_id, debit_amount, description
    ) VALUES (:v_gl_transaction_id, 1001, : p_initial_deposit, 'Cash received for account opening'
    );

    -- Credit Customer Deposit GL Account
    INSERT INTO core.gl_transaction_details (
        gl_transaction_id, gl_account_id, credit_amount, description
    ) VALUES (:v_gl_transaction_id, 2001, : p_initial_deposit, 'Customer deposit liability'
    );
END IF;
RETURN v_account_id;
END;
...
!!!RESOLVE EWI!!! /*** SSC-EWI-0068 - USER DEFINED FUNCTION WAS TRANSFORMED TO SNOWFLAKE PROCEDURE ***/!!!
CREATE OR REPLACE PROCEDURE core.process_transaction (
p_account_id INTEGER,
p_transaction_type_id INTEGER,
p_amount DECIMAL(18,2),
p_description TEXT,
p_reference_number VARCHAR(100) DEFAULT NULL !!!RESOLVE EWI!!! /*** SSC-EWI-0073 - PENDING FUNCTIONAL EQUIVALENCE REVIEW FOR 'ParameterDefaultExpr' NODE ***/!!!)
RETURNS BIGINT
LANGUAGE SQL
COMMENT = '{ "origin": "sf_sc", "name": "snowconvert", "version": {  "major": 1,  "minor": 16,  "patch": "1.0" }, "attributes": {  "component": "greenplum",  "convertedOn": "08/14/2025",  "domain": "snowflake" }}'
AS $$
...
IF (v_is_debit) THEN
    v_new_balance := v_current_balance - p_amount;
        ...
        IF (:v_new_balance < (-1 * : v_overdraft_limit)) THEN
                    CALL RAISE_MESSAGE_UDF('EXCEPTION', 'Insufficient funds. Available: %, Overdraft: %', array_construct(:v_available_balance, :v_overdraft_limit));
        END IF;
ELSE
    v_new_balance := v_current_balance + p_amount;
END IF;
...
SELECT 'TXN' || EXTRACT(YEAR FROM CURRENT_DATE()) ::TEXT || LPAD(core.transactions_transaction_id_seq.nextval /*** SSC-FDM-PG0009 - THE SEQUENCE NEXTVAL PROPERTY SNOWFLAKE DOES NOT GUARANTEE GENERATING SEQUENCE NUMBERS WITHOUT GAPS. ***/ ::TEXT, 12, '0') INTO
...
RETURNING transaction_id INTO v_transaction_id !!!RESOLVE EWI!!! /*** SSC-EWI-0073 - PENDING FUNCTIONAL EQUIVALENCE REVIEW FOR 'InsertIntoBody' NODE ***/!!!;
...
!!!RESOLVE EWI!!! /*** SSC-EWI-0068 - USER DEFINED FUNCTION WAS TRANSFORMED TO SNOWFLAKE PROCEDURE ***/!!!
CREATE OR REPLACE PROCEDURE core.transfer_funds (
...
IF (NOT EXISTS (SELECT 1 FROM
    core.accounts
        WHERE account_id = : p_from_account_id AND status_id = 1)) THEN
        CALL RAISE_MESSAGE_UDF('EXCEPTION', 'Source account not found or inactive');
END IF;
IF (NOT EXISTS (SELECT 1 FROM
    core.accounts
        WHERE account_id = : p_to_account_id AND status_id = 1)) THEN
        CALL RAISE_MESSAGE_UDF('EXCEPTION', 'Destination account not found or inactive');
END IF;
...
) !!!RESOLVE EWI!!! /*** SSC-EWI-0073 - PENDING FUNCTIONAL EQUIVALENCE REVIEW FOR 'jsonb_build_object' NODE ***/!!! INTO
        : v_result;
RETURN v_result;
END;
$$;
...
!!!RESOLVE EWI!!! /*** SSC-EWI-0068 - USER DEFINED FUNCTION WAS TRANSFORMED TO SNOWFLAKE PROCEDURE ***/!!!
CREATE OR REPLACE PROCEDURE loans.create_loan_application (
p_customer_id INTEGER,
...
FROM
        loans.loan_products
WHERE loan_product_id = : p_loan_product_id AND is_active = TRUE;
IF (NOT FOUND_UDF()) THEN
        CALL RAISE_MESSAGE_UDF('EXCEPTION', 'Invalid or inactive loan product');
END IF
IF (:p_requested_amount < : v_min_amount OR : p_requested_amount > : v_max_amount) THEN
        CALL RAISE_MESSAGE_UDF('EXCEPTION', 'Requested amount outside product limits: % to %', array_construct(:v_min_amount, :v_max_amount));
END IF;
IF (:p_requested_term_months < : v_min_term OR : p_requested_term_months > : v_max_term) THEN
        CALL RAISE_MESSAGE_UDF('EXCEPTION', 'Requested term outside product limits: % to % months', array_construct(:v_min_term, :v_max_term));
END IF;

-- Generate application number
SELECT 'LA' || EXTRACT(YEAR FROM CURRENT_DATE()) ::TEXT || LPAD(loans.loan_applications_application_id_seq.nextval /*** SSC-FDM-PG0009 - THE SEQUENCE NEXTVAL PROPERTY SNOWFLAKE DOES NOT GUARANTEE GENERATING SEQUENCE NUMBERS WITHOUT GAPS. ***/ ::TEXT, 8, '0') INTO
...
) RETURNING application_id INTO v_application_id !!!RESOLVE EWI!!! /*** SSC-EWI-0073 - PENDING FUNCTIONAL EQUIVALENCE REVIEW FOR 'InsertIntoBody' NODE ***/!!!;
RETURN v_application_id;
END;
$$;
-- 6. Calculate loan payment schedule
!!!RESOLVE EWI!!! /*** SSC-EWI-0068 - USER DEFINED FUNCTION WAS TRANSFORMED TO SNOWFLAKE PROCEDURE ***/!!!
CREATE OR REPLACE PROCEDURE loans.calculate_payment_schedule (
p_loan_id INTEGER)
RETURNS TABLE (
...
!!!RESOLVE EWI!!! /*** SSC-EWI-0068 - USER DEFINED FUNCTION WAS TRANSFORMED TO SNOWFLAKE PROCEDURE ***/!!!
CREATE OR REPLACE PROCEDURE loans.process_loan_payment (
p_loan_id INTEGER,
p_payment_amount DECIMAL(12,2),
p_payment_date DATE DEFAULT CURRENT_DATE !!!RESOLVE EWI!!! /*** SSC-EWI-0073 - PENDING FUNCTIONAL EQUIVALENCE REVIEW FOR 'ParameterDefaultExpr' NODE ***/!!!,
p_payment_method VARCHAR(20) DEFAULT 'AUTO_DEBIT' !!!RESOLVE EWI!!! /*** SSC-EWI-0073 - PENDING FUNCTIONAL EQUIVALENCE REVIEW FOR 'ParameterDefaultExpr' NODE ***/!!!)
RETURNS BIGINT
LANGUAGE SQL
COMMENT = '{ "origin": "sf_sc", "name": "snowconvert", "version": {  "major": 1,  "minor": 16,  "patch": "1.0" }, "attributes": {  "component": "greenplum",  "convertedOn": "08/14/2025",  "domain": "snowflake" }}'
AS $$
...
WHERE loan_id = : p_loan_id;
IF (NOT FOUND_UDF()) THEN
        CALL RAISE_MESSAGE_UDF('EXCEPTION', 'Loan not found');
END IF

-- Calculate interest for the period
v_days_interest := p_payment_date - COALESCE(v_last_payment_date,
                     (SELECT disbursement_date FROM
    loans.loans
        WHERE loan_id = : p_loan_id));
v_interest_amount := v_current_balance * v_interest_rate * v_days_interest / 365;

-- Calculate principal payment
v_principal_amount := LEAST_IGNORE_NULLS(p_payment_amount - v_interest_amount, v_current_balance);
...
) RETURNING payment_id INTO v_payment_id !!!RESOLVE EWI!!! /*** SSC-EWI-0073 - PENDING FUNCTIONAL EQUIVALENCE REVIEW FOR 'InsertIntoBody' NODE ***/!!!;
...
!!!RESOLVE EWI!!! /*** SSC-EWI-0068 - USER DEFINED FUNCTION WAS TRANSFORMED TO SNOWFLAKE PROCEDURE ***/!!!
CREATE OR REPLACE PROCEDURE trading.place_order (
p_portfolio_id INTEGER,
p_security_id INTEGER,
p_order_type_id INTEGER,
p_side VARCHAR(4),
p_quantity DECIMAL(18,6),
p_price DECIMAL(15,4) DEFAULT NULL !!!RESOLVE EWI!!! /*** SSC-EWI-0073 - PENDING FUNCTIONAL EQUIVALENCE REVIEW FOR 'ParameterDefaultExpr' NODE ***/!!!,
p_stop_price DECIMAL(15,4) DEFAULT NULL !!!RESOLVE EWI!!! /*** SSC-EWI-0073 - PENDING FUNCTIONAL EQUIVALENCE REVIEW FOR 'ParameterDefaultExpr' NODE ***/!!!)
RETURNS BIGINT
LANGUAGE SQL
COMMENT = '{ "origin": "sf_sc", "name": "snowconvert", "version": {  "major": 1,  "minor": 16,  "patch": "1.0" }, "attributes": {  "component": "greenplum",  "convertedOn": "08/14/2025",  "domain": "snowflake" }}'
AS $$
...
IF (NOT EXISTS (SELECT 1 FROM
    trading.portfolios
        WHERE portfolio_id = : p_portfolio_id AND is_active = TRUE)) THEN
        CALL RAISE_MESSAGE_UDF('EXCEPTION', 'Portfolio not found or inactive');
END IF;
-- Validate security exists
IF (NOT EXISTS (SELECT 1 FROM
    trading.securities
        WHERE security_id = : p_security_id AND is_active = TRUE)) THEN
        CALL RAISE_MESSAGE_UDF('EXCEPTION', 'Security not found or inactive');
END IF;
-- For buy orders, check buying power
IF (:p_side = 'BUY' AND p_price IS NOT NULL) THEN
    v_order_value := p_quantity * p_price;

    SELECT ta.buying_power INTO
    : v_buying_power
    FROM
    trading.trading_accounts ta
    JOIN
                        trading.portfolios p ON ta.trading_account_id = p.trading_account_id
    WHERE p.portfolio_id = : p_portfolio_id;
        IF (:v_order_value > : v_buying_power) THEN
    CALL RAISE_MESSAGE_UDF('EXCEPTION', 'Insufficient buying power. Required: %, Available: %', array_construct(:v_order_value, :v_buying_power));
        END IF;
END IF;

-- Generate order number
SELECT 'ORD' || EXTRACT(YEAR FROM CURRENT_DATE()) ::TEXT || LPAD(trading.orders_order_id_seq.nextval /*** SSC-FDM-PG0009 - THE SEQUENCE NEXTVAL PROPERTY SNOWFLAKE DOES NOT GUARANTEE GENERATING SEQUENCE NUMBERS WITHOUT GAPS. ***/ ::TEXT, 10, '0') INTO
        : v_order_number;
...
) RETURNING order_id INTO v_order_id !!!RESOLVE EWI!!! /*** SSC-EWI-0073 - PENDING FUNCTIONAL EQUIVALENCE REVIEW FOR 'InsertIntoBody' NODE ***/!!!;
RETURN v_order_id;
END;
$$;
-- 9. Execute trading order
!!!RESOLVE EWI!!! /*** SSC-EWI-0068 - USER DEFINED FUNCTION WAS TRANSFORMED TO SNOWFLAKE PROCEDURE ***/!!!
CREATE OR REPLACE PROCEDURE trading.execute_order (
p_order_id BIGINT,
p_execution_price DECIMAL(15,4),
p_execution_quantity DECIMAL(18,6),
p_execution_venue VARCHAR(50) DEFAULT 'INTERNAL' !!!RESOLVE EWI!!! /*** SSC-EWI-0073 - PENDING FUNCTIONAL EQUIVALENCE REVIEW FOR 'ParameterDefaultExpr' NODE ***/!!!)
RETURNS BIGINT
LANGUAGE SQL
COMMENT = '{ "origin": "sf_sc", "name": "snowconvert", "version": {  "major": 1,  "minor": 16,  "patch": "1.0" }, "attributes": {  "component": "greenplum",  "convertedOn": "08/14/2025",  "domain": "snowflake" }}'
AS $$
...
IF (NOT FOUND_UDF()) THEN
        CALL RAISE_MESSAGE_UDF('EXCEPTION', 'Order not found');
END IF
IF (:p_execution_quantity > : v_remaining_qty) THEN
        CALL RAISE_MESSAGE_UDF('EXCEPTION', 'Execution quantity exceeds remaining order quantity');
END IF;
...
) RETURNING execution_id INTO v_execution_id !!!RESOLVE EWI!!! /*** SSC-EWI-0073 - PENDING FUNCTIONAL EQUIVALENCE REVIEW FOR 'InsertIntoBody' NODE ***/!!!;
...
) RETURNING trade_id INTO v_trade_id !!!RESOLVE EWI!!! /*** SSC-EWI-0073 - PENDING FUNCTIONAL EQUIVALENCE REVIEW FOR 'InsertIntoBody' NODE ***/!!!;

-- Update holdings
SELECT holding_id, quantity INTO
        : v_holding_id,
        : v_current_qty
FROM
        trading.holdings
...
IF (v_holding_id IS NULL) THEN
        ...
        ) VALUES (:v_portfolio_id, : v_security_id, : v_new_qty, : p_execution_price
        );
    ELSE
...
!!!RESOLVE EWI!!! /*** SSC-EWI-0068 - USER DEFINED FUNCTION WAS TRANSFORMED TO SNOWFLAKE PROCEDURE ***/!!!
CREATE OR REPLACE PROCEDURE risk.calculate_var (
p_portfolio_id INTEGER,
p_confidence_level DECIMAL(5,4) DEFAULT 0.95 !!!RESOLVE EWI!!! /*** SSC-EWI-0073 - PENDING FUNCTIONAL EQUIVALENCE REVIEW FOR 'ParameterDefaultExpr' NODE ***/!!!,
p_time_horizon INTEGER DEFAULT 1 !!!RESOLVE EWI!!! /*** SSC-EWI-0073 - PENDING FUNCTIONAL EQUIVALENCE REVIEW FOR 'ParameterDefaultExpr' NODE ***/!!!)
...
!!!RESOLVE EWI!!! /*** SSC-EWI-0068 - USER DEFINED FUNCTION WAS TRANSFORMED TO SNOWFLAKE PROCEDURE ***/!!!
CREATE OR REPLACE PROCEDURE analytics.calculate_customer_profitability (
p_customer_id INTEGER,
p_start_date DATE,
p_end_date DATE)
RETURNS VARIANT
...
SELECT
        COALESCE(SUM(fee_amount), 0) INTO
        : v_fee_income
FROM
        core.transactions t
JOIN
...
SELECT
        COALESCE(SUM(lp.interest_amount), 0) INTO
        : v_interest_income
FROM
        loans.loan_payments lp
JOIN
    loans.loans l ON lp.loan_id = l.loan_id
WHERE l.customer_id = : p_customer_id
AND lp.payment_date BETWEEN : p_start_date AND : p_end_date;

-- Calculate interest expense on deposits (simplified)
SELECT
        COALESCE(SUM(current_balance * 0.01 / 365 * (:p_end_date - : p_start_date)), 0) INTO
        : v_interest_expense
FROM
        core.accounts
WHERE customer_id = : p_customer_id
AND account_type_id IN (SELECT account_type_id FROM
            core.account_types
...
!!!RESOLVE EWI!!! /*** SSC-EWI-0068 - USER DEFINED FUNCTION WAS TRANSFORMED TO SNOWFLAKE PROCEDURE ***/!!!
CREATE OR REPLACE PROCEDURE core.run_daily_batch ()
RETURNS TEXT
LANGUAGE SQL
COMMENT = '{ "origin": "sf_sc", "name": "snowconvert", "version": {  "major": 1,  "minor": 16,  "patch": "1.0" }, "attributes": {  "component": "greenplum",  "convertedOn": "08/14/2025",  "domain": "snowflake" }}'
AS $$
...
FROM
        core.accounts a
JOIN
    core.account_types at ON a.account_type_id = at.account_type_id
LEFT JOIN
    core.interest_calculations ic ON a.account_id = ic.account_id
...
v_processed_accounts := SQLROWCOUNT;

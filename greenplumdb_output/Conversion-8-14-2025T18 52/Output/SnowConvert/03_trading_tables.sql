...
USE SCHEMA trading /*, core, public*/;

-- 26. Market Data Providers
CREATE TABLE trading.market_data_providers (
    provider_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE trading.security_types (
    security_type_id INTEGER PRIMARY KEY IDENTITY,
...
--** SSC-FDM-GP0001 - THE PERFORMANCE OF THE CLUSTER BY MAY VARY COMPARED TO THE PERFORMANCE OF DISTRIBUTED BY **
CLUSTER BY (security_type_id)
COMMENT = '{ "origin": "sf_sc", "name": "snowconvert", "version": {  "major": 1,  "minor": 16,  "patch": "1.0" }, "attributes": {  "component": "greenplum",  "convertedOn": "08/14/2025",  "domain": "snowflake" }}'
;

-- 29. Securities Master
CREATE TABLE trading.securities (
    security_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE trading.market_data (
    market_data_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE trading.real_time_quotes (
    quote_id INTEGER PRIMARY KEY IDENTITY,
    security_id INTEGER REFERENCES trading.securities (security_id),
...
CREATE TABLE trading.trading_accounts (
    trading_account_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE trading.portfolios (
    portfolio_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE trading.holdings (
    holding_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE trading.order_types (
    order_type_id INTEGER PRIMARY KEY IDENTITY,
    type_code VARCHAR(10) NOT NULL UNIQUE,
    type_name VARCHAR(50) NOT NULL,
    description TEXT,
    is_active BOOLEAN DEFAULT TRUE,
...
--** SSC-FDM-GP0001 - THE PERFORMANCE OF THE CLUSTER BY MAY VARY COMPARED TO THE PERFORMANCE OF DISTRIBUTED BY **
CLUSTER BY (order_type_id)
COMMENT = '{ "origin": "sf_sc", "name": "snowconvert", "version": {  "major": 1,  "minor": 16,  "patch": "1.0" }, "attributes": {  "component": "greenplum",  "convertedOn": "08/14/2025",  "domain": "snowflake" }}'
;

-- 36. Order Status
CREATE TABLE trading.order_status (
    status_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE trading.orders (
    ...
    side VARCHAR(4) NOT NULL
                             !!!RESOLVE EWI!!! /*** SSC-EWI-0035 - CHECK STATEMENT NOT SUPPORTED ***/!!! CHECK (side IN ('BUY', 'SELL')),
    quantity DECIMAL(18,6) NOT NULL,
    price DECIMAL(15,4),
...
CREATE TABLE trading.trades (
    trade_id INTEGER PRIMARY KEY IDENTITY,
...
CREATE TABLE trading.corporate_action_types (
    action_type_id INTEGER PRIMARY KEY IDENTITY,
    type_code VARCHAR(10) NOT NULL UNIQUE,
    type_name VARCHAR(50) NOT NULL,
    description TEXT,
...
--** SSC-FDM-GP0001 - THE PERFORMANCE OF THE CLUSTER BY MAY VARY COMPARED TO THE PERFORMANCE OF DISTRIBUTED BY **
CLUSTER BY (action_type_id)
COMMENT = '{ "origin": "sf_sc", "name": "snowconvert", "version": {  "major": 1,  "minor": 16,  "patch": "1.0" }, "attributes": {  "component": "greenplum",  "convertedOn": "08/14/2025",  "domain": "snowflake" }}'
;

-- 41. Corporate Actions
CREATE TABLE trading.corporate_actions (
    corporate_action_id INTEGER PRIMARY KEY IDENTITY,
    security_id INTEGER REFERENCES trading.securities (security_id),
...
CREATE TABLE trading.watchlists (
    watchlist_id INTEGER PRIMARY KEY IDENTITY,
    customer_id INTEGER REFERENCES core.customers (customer_id),
...
CREATE TABLE trading.watchlist_securities (
    watchlist_security_id INTEGER PRIMARY KEY IDENTITY,
...
--** SSC-FDM-GP0001 - THE PERFORMANCE OF THE CLUSTER BY MAY VARY COMPARED TO THE PERFORMANCE OF DISTRIBUTED BY **
CLUSTER BY (watchlist_id)
COMMENT = '{ "origin": "sf_sc", "name": "snowconvert", "version": {  "major": 1,  "minor": 16,  "patch": "1.0" }, "attributes": {  "component": "greenplum",  "convertedOn": "08/14/2025",  "domain": "snowflake" }}'
;

-- 44. Margin Requirements
CREATE TABLE trading.margin_requirements (
    margin_requirement_id INTEGER PRIMARY KEY IDENTITY,
    security_id INTEGER REFERENCES trading.securities (security_id),
...
CREATE TABLE trading.position_valuations (
    valuation_id INTEGER PRIMARY KEY IDENTITY,
    portfolio_id INTEGER REFERENCES trading.portfolios (portfolio_id),
    security_id INTEGER REFERENCES trading.securities (security_id),
...
CREATE TABLE trading.investment_research (
    research_id INTEGER PRIMARY KEY IDENTITY,
    security_id INTEGER REFERENCES trading.securities (security_id),
...
CREATE TABLE trading.broker_dealers (
    broker_dealer_id INTEGER PRIMARY KEY IDENTITY,
...
--** SSC-FDM-GP0001 - THE PERFORMANCE OF THE CLUSTER BY MAY VARY COMPARED TO THE PERFORMANCE OF DISTRIBUTED BY **
CLUSTER BY (broker_dealer_id)
COMMENT = '{ "origin": "sf_sc", "name": "snowconvert", "version": {  "major": 1,  "minor": 16,  "patch": "1.0" }, "attributes": {  "component": "greenplum",  "convertedOn": "08/14/2025",  "domain": "snowflake" }}'
;

-- 48. Prime Brokerage
CREATE TABLE trading.prime_brokerage (
    prime_brokerage_id INTEGER PRIMARY KEY IDENTITY,
    portfolio_id INTEGER REFERENCES trading.portfolios (portfolio_id),
...
CREATE TABLE trading.options (
    option_id INTEGER PRIMARY KEY IDENTITY,
    underlying_security_id INTEGER REFERENCES trading.securities (security_id),
    option_symbol VARCHAR(20) NOT NULL UNIQUE,
    option_type VARCHAR(4) NOT NULL
                                    !!!RESOLVE EWI!!! /*** SSC-EWI-0035 - CHECK STATEMENT NOT SUPPORTED ***/!!! CHECK (option_type IN ('CALL', 'PUT')),
...
--** SSC-FDM-GP0001 - THE PERFORMANCE OF THE CLUSTER BY MAY VARY COMPARED TO THE PERFORMANCE OF DISTRIBUTED BY **
CLUSTER BY (underlying_security_id)
COMMENT = '{ "origin": "sf_sc", "name": "snowconvert", "version": {  "major": 1,  "minor": 16,  "patch": "1.0" }, "attributes": {  "component": "greenplum",  "convertedOn": "08/14/2025",  "domain": "snowflake" }}'
;

-- 50. Futures
CREATE TABLE trading.futures (
    future_id INTEGER PRIMARY KEY IDENTITY,
